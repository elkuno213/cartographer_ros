...
visualization_msgs::Marker CreateLandmarkMarker(
  int landmark_index,
...
void MapBuilderBridge::LoadState(
  ...
  CHECK_EQ(
    state_filename.substr(std::max<int>(state_filename.size() - suffix.size(), 0)),
    suffix
...
int MapBuilderBridge::AddTrajectory(
  ...
  // Add trajectory into map builder and get back trajectory_id
  ...
  // Make sure there is no trajectory with 'trajectory_id' yet in sensor bridges.
...
bool MapBuilderBridge::SerializeState(
  const std::string& filename,
  const bool include_unfinished_submaps
) {
  return map_builder_->SerializeStateToFile(
...
void MapBuilderBridge::HandleSubmapQuery(
  ...
  // Retrieve submap response from proto
  ...
  // Fetch into ROS response
  response.submap_version = response_proto.submap_version();
  for (const auto& texture_proto : response_proto.textures()) {
    response.textures.emplace_back();
    auto& texture = response.textures.back();
    texture.cells.insert(
      texture.cells.begin(),
      texture_proto.cells().begin(),
...
void MapBuilderBridge::HandleTrajectoryQuery(
  ...
  // This query is safe if the trajectory doesn't exist (returns 0 pose).
...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList() {
    ...
    if (constraint.tag == cartographer::mapping::PoseGraphInterface::Constraint::INTER_SUBMAP) {
      if (constraint.node_id.trajectory_id == constraint.submap_id.trajectory_id) {
        trajectory_to_last_inter_submap_constrained_node[constraint.node_id.trajectory_id]
          = std::max(
              trajectory_to_last_inter_submap_constrained_node.at(constraint.node_id.trajectory_id),
  ...
  for (const int trajectory_id : node_poses.trajectory_ids()) {
    ...
    last_inter_submap_constrained_node = std::max(
...
void MapBuilderBridge::OnLocalSlamResult(
  ...
  std::shared_ptr<const LocalTrajectoryData::LocalSlamData> local_slam_data
    = std::make_shared<LocalTrajectoryData::LocalSlamData>(LocalTrajectoryData::LocalSlamData{
        time,
        local_pose,
