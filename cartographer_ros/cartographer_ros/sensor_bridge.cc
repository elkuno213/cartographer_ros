...
const std::string& CheckNoLeadingSlash(const std::string& frame_id) {
  if (frame_id.size() > 0) {
    CHECK_NE(frame_id[0], '/')
      ...
      << " should not start with a /. See 1.7 in http://wiki.ros.org/tf2/Migration.";
  }
  return frame_id;
}
...
SensorBridge::SensorBridge(
  ...
  const bool ignore_out_of_order_messages,
...
bool SensorBridge::IgnoreMessage(
...
void SensorBridge::HandleOdometryMessage(
  ...
  if (odometry_data != nullptr) {
    if (IgnoreMessage(sensor_id, odometry_data->time)) {
      LOG(WARNING)
        << "Ignored odometry message from sensor " << sensor_id
...
void SensorBridge::HandleImuMessage(
  ...
  if (imu_data != nullptr) {
    if (IgnoreMessage(sensor_id, imu_data->time)) {
      LOG(WARNING)
        << "Ignored IMU message from sensor " << sensor_id
...
void SensorBridge::HandleLaserScan(
  ...
  for (int i = 0; i != num_subdivisions_per_laser_scan_; ++i) {
    ...
    if (it != sensor_to_previous_subdivision_time_.end() &&
      ...
      LOG(WARNING)
        << "Ignored subdivision of a LaserScan message from sensor "
...
void SensorBridge::HandleRangefinder(
  ...
  if (sensor_to_tracking != nullptr) {
    if (IgnoreMessage(sensor_id, time)) {
      LOG(WARNING)
        << "Ignored Rangefinder message from sensor " << sensor_id
