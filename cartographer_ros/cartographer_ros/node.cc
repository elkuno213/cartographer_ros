...
Node::Node(
  ...
  if (collect_metrics) {
    ...
    cartographer::metrics::RegisterAllMetrics(metrics_registry_.get());
  }
  ...
  service_servers_.push_back(
    node_handle_.advertiseService(
      ...
      &Node::HandleGetTrajectoryStates,
      this
    )
  );
  service_servers_.push_back(
  ...
  for (const auto& entry : map_builder_bridge_.GetLocalTrajectoryData()) {
    ...
    if (trajectory_data.local_slam_data->time != extrapolator.GetLastPoseTime()) {
      if (scan_matched_point_cloud_publisher_.getNumSubscribers() > 0) {
        ...
        scan_matched_point_cloud_publisher_.publish(ToPointCloud2Message(
          cartographer::common::ToUniversal(trajectory_data.local_slam_data->time),
          node_options_.map_frame,
          cartographer::sensor::TransformTimedPointCloud(point_cloud, trajectory_data.local_to_map.cast<float>())
        ));
      }
    ...
    const Rigid3d tracking_to_local = [&] {
      if (trajectory_data.trajectory_options.publish_frame_projected_to_2d) {
        return cartographer::transform::Embed3D(cartographer::transform::Project2D(tracking_to_local_3d));
      }
...
void Node::LaunchSubscribers(
  ...
  for (const auto& topic : ComputeRepeatedTopicNames(kMultiEchoLaserScanTopic, options.num_multi_echo_laser_scans)) {
    subscribers_[trajectory_id].push_back(
      {
...
cartographer_ros_msgs::StatusResponse Node::TrajectoryStateToStatus(
  ...
  status_response.message = absl::StrCat(
    "Trajectory ",
    trajectory_id,
    " is in '",
...
void Node::RunFinalOptimization() {
  // Finish any active trajectory before starting the optimization.
  {
    for (const auto& entry : map_builder_bridge_.GetTrajectoryStates()) {
      const int trajectory_id = entry.first;
      if (entry.second == TrajectoryState::ACTIVE) {
